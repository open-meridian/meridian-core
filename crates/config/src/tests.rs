...
fn with_ada() -> MemoryStore {
    let store = MemoryStore::new();
    store
        .put_local_account(&crate::LocalAccount {
            name: "ada".into(),
...
fn the_decoy_hash_is_a_hash() {
    // It is verified against when no account matches, so that an unknown name
    // costs the same time as a known one. A decoy that does not parse fails
    // fast, which restores exactly the timing difference it exists to remove
    // -- and nothing else would notice, because the answer is the same either
    // way.
    let store = MemoryStore::new();
    let started = std::time::Instant::now();
    let absent = authenticate_local_account(&store, "nobody", "whatever", T0);
    ...
    let store = with_ada();
    let started = std::time::Instant::now();
    authenticate_local_account(&store, "ada", "wrong", T0);
    let known = started.elapsed();

    // Generous: this asserts the same order of magnitude, not a constant
    // time. What it catches is the decoy being skipped entirely, which is
...
fn the_right_password_signs_in_and_brings_its_groups() {
    let signed_in = authenticate_local_account(&with_ada(), "ada", "correct horse battery", T0);

    assert!(signed_in.authenticated, "{}", signed_in.refusal_reason);
    assert_eq!(signed_in.subject, "local|ada");
    assert_eq!(signed_in.display_name, "Ada Park");
    assert_eq!(signed_in.groups, vec!["meridian-admins".to_string()]);
}

#[test]
fn a_name_that_is_not_there_reads_the_same_as_a_wrong_password() {
    let store = with_ada();
    let wrong = authenticate_local_account(&store, "ada", "not it", T0);
    let absent = authenticate_local_account(&store, "nobody", "not it", T0);

    assert!(!wrong.authenticated && !absent.authenticated);
    // Identical, deliberately: anything else tells a stranger who works here.
    assert_eq!(wrong.refusal_reason, absent.refusal_reason);
}
...
fn enough_wrong_passwords_lock_the_account() {
    ...
    for _ in 0..LOCK_AFTER {
        assert!(!authenticate_local_account(&store, "ada", "not it", T0).authenticated);
    }

    // And now the right one does not work either, which is the whole point:
    // a lock that the correct password lifts is not a lock.
    let refused = authenticate_local_account(&store, "ada", "correct horse battery", T0);
    assert!(!refused.authenticated);
    assert!(
        refused.refusal_reason.contains("too many attempts"),
        "{refused:?}"
    );
}
...
fn a_lock_lifts_by_itself() {
    let store = with_ada();
    for _ in 0..LOCK_AFTER {
        authenticate_local_account(&store, "ada", "not it", T0);
    }

    let after = T0 + LOCK_FOR_NS + 1;
    let signed_in = authenticate_local_account(&store, "ada", "correct horse battery", after);
...
fn signing_in_clears_the_count_so_a_typo_is_not_cumulative() {
    ...
    assert!(authenticate_local_account(&store, "ada", "correct horse battery", T0).authenticated);

    // Without the clear, one more mistake next week would lock the account
    // on a count nobody remembers accumulating.
    for _ in 0..(LOCK_AFTER - 1) {
        authenticate_local_account(&store, "ada", "not it", T0);
    }
    assert!(authenticate_local_account(&store, "ada", "correct horse battery", T0).authenticated);
...
fn an_empty_password_never_matches() {
    // No special case is needed here, unlike LDAP, where an empty password is
    // a bind the server may answer with success. A stored hash simply does
    // not match one. Asserted so that the difference between the two branches
    // is written down rather than assumed.
    assert!(!authenticate_local_account(&with_ada(), "ada", "", T0).authenticated);
}
