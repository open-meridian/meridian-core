//! The ledger's migration history: what exists, what has run, and what a
...
//! ledger holds statements and positions nothing can reconstruct, and
...
//!
//! [`apply`] runs once per release, from `meridian-runtime migrate`. Starting
...
pub fn verify(applied: Option<i64>) -> Result<()> {
    ...
    match applied {
        None => Err(StoreError::Unavailable(format!(
            "the ledger's database has no schema; it is at no version and this binary \
             expects {latest}. Run `meridian-runtime migrate` before starting."
        ))),
        Some(at) if at < latest => Err(StoreError::Unavailable(format!(
            "the ledger's database is at schema version {at} and this binary expects \
             {latest}. Run `meridian-runtime migrate` before starting."
        ))),
        ...
        Some(at) if at > latest => Err(StoreError::Unavailable(format!(
            "the ledger's database is at schema version {at}, which is newer than this \
