...
/// Decision 018: where a firm has no directory of its own, the deployment holds
/// the accounts. This is the only sign-in carrying a bus message, because those
/// accounts live in the conductor's store and the dashboard has no database of
/// its own to check a password against.
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct AuthenticateLocalAccountRequest {
    #[prost(string, tag = "1")]
    pub name: ::prost::alloc::string::String,
    /// Sealed to the conductor rather than sent in the clear, for the reason
    /// first run seals credentials to its Job: the broker sees a deployment's
    /// every message, and a password on it is a password the broker holds
    /// (spec/installation-and-first-run, ruling 4).
    #[prost(message, optional, tag = "2")]
    pub password: ::core::option::Option<SealedCredential>,
}
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct LocalAccountAuthentication {
    #[prost(bool, tag = "1")]
    pub authenticated: bool,
    /// One sentence, and the same one whether the name is unknown or the password
    /// is wrong: telling them apart lets anybody who can reach the sign-in page
    /// enumerate a firm's staff. Being locked out is said plainly, because a
    /// person who is not told will keep trying and cannot tell it from a wrong
    /// password.
    #[prost(string, tag = "2")]
    pub refusal_reason: ::prost::alloc::string::String,
    /// `{issuer}|{subject}`, as every login is. Half of every permission ever
    /// granted, so it is settled once and never changed.
    #[prost(string, tag = "3")]
    pub subject: ::prost::alloc::string::String,
    #[prost(string, tag = "4")]
    pub display_name: ::prost::alloc::string::String,
    /// What the access model joins, exactly as it joins a directory's groups. An
    /// account this deployment holds is not a different kind of person.
    #[prost(string, repeated, tag = "5")]
