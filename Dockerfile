...
COPY crates/ crates/
# The contract the sidecar and the broker grant from, compiled in
# (decisions/020): who publishes and subscribes to what, and which names are
# roles. Generated by meridian-design; the only part of deploy/ the binaries
# read.
COPY deploy/topics.tsv deploy/roles.tsv deploy/
